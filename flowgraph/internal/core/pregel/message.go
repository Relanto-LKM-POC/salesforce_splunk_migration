...
// ConsumeMessages returns all pending messages for the vertex and removes them from the aggregator.
func (ma *MessageAggregator) ConsumeMessages(vertexID string) []*Message {
	ma.mu.Lock()
	defer ma.mu.Unlock()
	msgs := ma.messages[vertexID]
	if len(msgs) == 0 {
		delete(ma.messages, vertexID) // Ensure empty buckets do not linger
		return nil
	}

	if ma.combiner != nil && len(msgs) > 1 {
		if combined := ma.combiner.Combine(msgs); combined != nil {
...
// HasPending reports whether there are queued messages for the given vertex.
func (ma *MessageAggregator) HasPending(vertexID string) bool {
	ma.mu.RLock()
	defer ma.mu.RUnlock()
	return len(ma.messages[vertexID]) > 0
}
